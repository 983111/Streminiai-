...
// ─────────────────────────────────────────────────────────────────────────────
// Design tokens — mirrors home_screen.dart exactly:
...
const _userBubble  = Color(0xFF0C1C2B);   // deep navy, same accent family
...
class _ChatScreenState extends ConsumerState<ChatScreen>
  ...
  File?   _selectedFile;
  String? _base64File, _mimeType, _fileName;
  ...
  Future<void> _handlePick(String type) async {
    switch (type) {
      ...
      case 'image':
        ...
        if (img != null) _processAttachment(File(img.path));
        break;
      case 'file':
        final r = await FilePicker.platform.pickFiles();
        if (r?.files.single.path != null)
          _processAttachment(File(r!.files.single.path!));
        break;
    }
  }
  ...
  Future<void> _processAttachment(File file) async {
    try {
      ...
      setState(() {
        ...
        _mimeType     = lookupMimeType(file.path) ?? 'application/octet-stream';
  ...
  void _clearAttach() => setState(() {
        _selectedFile = null; _base64File = null;
        _mimeType = null; _fileName = null;
      });
  ...
  Widget _filePreview() => Container(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Text(_fileName ?? 'Attached File',
            ...
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                    color: docCtx != null ? _accent.withOpacity(0.35) : _border),
                ),
                child: TextField(
